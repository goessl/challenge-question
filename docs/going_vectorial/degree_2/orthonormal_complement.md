...
Two particles in degree two (dimensionality three) is also a special case:
...
$$
    \begin{aligned}
        ...
        &= \text{tr}\,\hat{T}-\frac{1}{8}\int_\mathbb{R}\frac{\left(\frac{\mathrm{d}}{\mathrm{d}x}\left(\sum_j|b_j(x)|^2-n(x)\right)\right)^2}{\sum_j|b_j(x)|^2-n(x)}\,\mathrm{d}x \\
        &= \text{tr}\,\hat{T}-\frac{1}{8}\int_\mathbb{R}\frac{\left(\sum_j2b_j(x)b_j'(x)-n'(x)\right)^2}{\sum_j|b_j(x)|^2-n(x)}\,\mathrm{d}x
    \end{aligned}
$$
...
!!! danger "Square root"
    ...
    Might be easier to just calculate the hole wavefunction as $\sqrt{\sum_j|b_j(x)|^2-n(x)}$.
