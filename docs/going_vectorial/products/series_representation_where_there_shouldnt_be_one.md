...
We all still know from kindergarden that $L^2(\mathbb{R})$ is the only $L^p(\mathbb{R})$ space that admits an inner product and is therrfore the only Hilbert space of all $L^p(\mathbb{R})$ spaces.
...
But no, dear reader, don't despair, the probability density function (at least ours) has actually a finite vectorial representation via the polynomials. Now we just make it a little more natural:
...
## Products of Hermite function series

Further for Hermite function series:

$$
    \begin{aligned}
        &f(x)g(x) &&\mid f(x)=\sum_if_ih_i(x), \ g(x)=\sum_jg_jh_j(x) \\
        &= \sum_if_ih_i(x)\sum_jg_jh_j(x) \\
        &= \sum_{ij}f_ig_jh_i(x)h_j(x) &&\mid h_i(x)h_j(x)=h_0(x)\cdots \\
        &= \sum_{ij}f_ig_jh_0(x)\sum_{k=0}^{\min\{i,j\}}\frac{\sqrt{i!j!(i+j-2k)!}}{k!(i-k)!(j-k)!}h_{i+j-2k}(x) \\
        &= h_0(x)\sum_{ij}f_ig_j\sum_{k=0}^{\min\{i,j\}}\frac{\sqrt{i!j!(i+j-2k)!}}{k!(i-k)!(j-k)!}h_{i+j-2k}(x)
    \end{aligned}
$$
...
!!! danger "TODO"
    ...
    - Examples for squares (like $\rho=\phi^2$)
    - Dimensionality (like degree n wavefunction squared gives degree m density vector)
    - Analysis of the $\vec{g}(\rho)$ function (linearity, kernel, ...)
